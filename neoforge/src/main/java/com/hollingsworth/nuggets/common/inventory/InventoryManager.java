...
public class InventoryManager {
    ...
    public InventoryManager(IItemHandler handler){
        this(List.of(new FilterableItemHandler(handler)));
    }
    ...
    public boolean hasItemCount(Predicate<ItemStack> predicate, int count){
        int remaining = count;
        for (FilterableItemHandler filterable : getInventory()) {
            for (int i = 0; i < filterable.getHandler().getSlots(); i++) {
                ItemStack stack = filterable.getHandler().getStackInSlot(i);
    ...
    public Collection<FilterablePreference> preferredForStack(ItemStack stack, boolean includeInvalid) {
        PriorityQueue<FilterablePreference> filtered = new PriorityQueue<>((o1, o2) -> {
            SortPref pref1 = o1.pref();
            SortPref pref2 = o2.pref();
            return pref2.ordinal() - pref1.ordinal();
        });
        for(FilterableItemHandler filterableItemHandler : getInventory()){
            SortPref sortPref = filterableItemHandler.getHighestPreference(stack);
            if(includeInvalid || sortPref != SortPref.INVALID){
    ...
    public FilterableItemHandler highestPrefInventory(List<FilterableItemHandler> inventories, Predicate<ItemStack> predicate, InteractType type) {
        SortPref highestPref = SortPref.INVALID;
        ...
        for (FilterableItemHandler wrapper : inventories) {
            ...
            for (int i = 0; i < maxSlotForType(wrapper, type); i++) {
                ...
                SortPref foundPref = result.sortPref();
                if (!result.valid()) {
                    continue;
                }
    ...
    public record FilterablePreference(FilterableItemHandler handler, SortPref pref) {
    }
}
